...
#define ERROR 5000 // out of range of sensor values
/*!
...
size_t fifo_pop(fifo_t* fifo) {
    ...
    int popped_val = fifo->buffer[fifo->tail];
...
void fifo_print(fifo_t* fifo) {
    ...
    printf("Printing FIFO with %d elements", fifo->count);
    printf("Oldest value (buffer[%d]): %d\n ", fifo->head, fifo->buffer[fifo->head]);
    printf("Newest value (buffer[%d]): %d\n", fifo->tail, fifo->buffer[fifo->tail]);
